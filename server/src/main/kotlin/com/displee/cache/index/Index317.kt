...
class Index317(origin: CacheLibrary, id: Int, raf: RandomAccessFile) : Index(origin, id, raf) {
    ...
    override fun init() {
        ...
        if (id != CONFIG_INDEX && id < VERSION_FILES.size) {
            ...
            if (ENABLE_CRCS) crcs = readArchiveProperties(CRC_FILES[id - 1], BufferType.INT)
        }
        for (i in 0 until archiveLength) {
            val archive = Archive317(i)
            archives[i] = archive
            if (versions == null || (ENABLE_CRCS && crcs == null) || i >= versions.size) {
                continue
            }
            archive.revision = versions[i]
            if (ENABLE_CRCS) archive.crc = crcs!![i]
        }
    }
    ...
    companion object {
        ...
        private const val ENABLE_CRCS = false
    }
