...
data class Model(val id: Int) {
    ...
    private fun encodeUnversionedTriangleInfo(
        ...
        if (triangleCount <= 0) return
        val triangleColors = requireNotNull(this.triangleColors)
        val hasTriangleRenderTypes =
            triangleRenderTypes != null || this.textureCoordinates != null || this.triangleTextures != null
        ...
        private fun Model.append(models: Collection<Model>) {
            ...
            for (model in models) {
                for (index in 0 until model.triangleCount) {
                    if (model.faceBillboards != null) {
                        for (i in 0 until model.faceBillboards!!.size) {
                            ...
                            billboards!![billboardsCount++] = FaceBillboard(
                                billboard.id,
                                billboard.face + triangleCount,
                                billboard.skin,
                                billboard.distance
                            )
                        }
                    ...
                    if (hasTextureCoordinates) {
                        if (model.textureCoordinates != null && model.textureCoordinates!![index] != -1) {
                            textureCoordinates!![triangleCount] =
                                (textureTriangleCount + model.textureCoordinates!![index]).toByte().toInt()
                        } else {
                ...
                if (emitters != null) {
                    for (index in emitters.indices) {
                        ...
                        emitters[emittersCount] =
                            EmissiveTriangle(emitter.emitter, emitter.face, vertex1, vertex2, vertex3, emitter.priority)
                        emittersCount++
                    }
                }
                if (effectors != null) {
            ...
            for (model in models) {
                ...
                for (index in 0 until model.textureTriangleCount) {
                    ...
                    if (type.toInt() == 0) {
                        textureTriangleVertex1!![textureTriangleCount] =
                            findVertex(model, model.textureTriangleVertex1!![index]).toShort().toInt()
                        textureTriangleVertex2!![textureTriangleCount] =
                            findVertex(model, model.textureTriangleVertex2!![index]).toShort().toInt()
                        textureTriangleVertex3!![textureTriangleCount] =
                            findVertex(model, model.textureTriangleVertex3!![index]).toShort().toInt()
                    }
                    if (type in 1..3) {
